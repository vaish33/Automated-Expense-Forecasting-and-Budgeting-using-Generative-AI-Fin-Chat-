...
def generate_budgeting_recommendations_from_ai(category, forecast, historical_data, user_prompt):
    prompt_text = (
        f"The forecasted expenses for {category} over the next few months are:\n"
        f"{forecast.to_string(index=False)}\n"
        f"Based on this forecast and historical data:\n"
        f"{historical_data}\n"
        f"{user_prompt}\n"
        f"Provide detailed budgeting recommendations."
    )
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
        ...
        st.error(f"OpenAI API error: {str(e)}")
        return "Unable to generate budgeting recommendations due to API quota limitations."
...
if st.sidebar.button('Generate Forecast'):
    ...
    # Generate budgeting recommendations using AI
    try:
        recommendations = generate_budgeting_recommendations_from_ai(category, forecast_series, historical_data, user_prompt)
        st.subheader('Budgeting Recommendations')
        st.write(recommendations)
    except Exception as e:
        st.write(f"Error generating budgeting recommendations: {e}")
